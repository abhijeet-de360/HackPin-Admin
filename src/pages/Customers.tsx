...
const Customers = () => {
  ...
  const [customersList, setCustomersList] = useState<Customer[]>(customersData as Customer[]);
  ...
  const customerStats = useMemo(() => {
    ...
    customers.forEach((customer) => {
      ...
      const dueOrders = customerOrders.filter((order) => order.grand_total - order.advance_amount > 0);
  ...
  const handleExportCSV = () => {
    ...
    if (customersToExport.length === 0) {
      toast({
        ...
        description: "Please select customers or clear the selection to export all",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="rounded-md border bg-card">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-40">App Account</TableHead>
                <TableHead className="w-24">Tag</TableHead>
                <TableHead className="w-32 text-right">Due Orders</TableHead>
              ...
              {visibleCustomers.map((customer) => {
                ...
                return (
                  <TableRow key={customer.id}>
                    ...
                    <TableCell className="text-right">{stats.dueOrderCount}</TableCell>
                    <TableCell className="text-right">
                      <span className={stats.totalDue > 0 ? "text-orange-600" : ""}>₹{stats.totalDue.toFixed(2)}</span>
                    </TableCell>
                  </TableRow>
                );
              })}
          ...
          {displayCount < filteredCustomers.length && (
            <div className="text-center py-4 text-sm text-muted-foreground border-t">
              Showing {displayCount} of {filteredCustomers.length} customers • Scroll for more
            </div>
          )}
          {displayCount >= filteredCustomers.length && filteredCustomers.length > 50 && (
            <div className="text-center py-4 text-sm text-muted-foreground border-t">
